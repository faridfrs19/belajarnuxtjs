  ...
  <div class="relative select-none">
    ...
    <div v-show="isShow" class="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-xl w-full absolute top-14">
      <ul class="py-2 text-sm text-gray-700">
        <li>
          <span class="block px-4 py-2 hover:bg-primary hover:text-blue transition duration-200 cursor-pointer" @click="$emit('selectedCategory', '')">All</span>
        </li>
        <li v-for="(item, index) in category" :key="index">
          <span class="block px-4 py-2 hover:bg-primary hover:text-blue transition duration-200 cursor-pointer" @click="$emit('selectedCategory', item.name)">{{ item?.name }}</span>
        </li>
      </ul>
    </div>
  </div>
